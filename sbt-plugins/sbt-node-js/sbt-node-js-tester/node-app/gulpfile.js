...
gulp.task('bundle-tests', function() {
  return gulp.src(paths.jsTest)
    ...
    .pipe(concat('bundled_tests.js'))
    .pipe(gulp.dest(paths.tmp));
});

gulp.task('watch-bundled-tests', ['bundle-tests'], function() {
  return gulp.watch(paths.scripts, ['bundle-tests']);
});

var karmaTask = function(taskName, action, deps) {
  return gulp.task(taskName, deps, function() {
    return gulp.src(paths.bundledTests)
      .pipe(karma({
...
karmaTask('test', 'run', ['bundle-tests']);

/** Run karma tests when files change */
karmaTask('test-watch', 'watch', ['watch-bundled-tests']);
