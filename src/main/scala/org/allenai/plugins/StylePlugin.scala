  ...
  override def projectSettings: Seq[Def.Setting[_]] =
    ...
    ScalastylePlugin.projectSettings ++
      ...
      Seq(
        ...
        compileInputs.in(Test, compile) := Def.taskDyn {
          StyleKeys.styleCheck.in(Test).map(_ => compileInputs.in(Test, compile).value)
        }.value,
        compileInputs.in(Compile, compile) := Def.taskDyn {
          StyleKeys.styleCheck.in(Compile).map(_ => compileInputs.in(Compile, compile).value)
        }.value
      )
